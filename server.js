...
async function sendLeadEmail(lead) {
  ...
  if (!smtpUser || !toEmail) {
    throw new Error(
      'SMTP_USER или TO_EMAIL не заполнены',
    );
  }
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const phone = normalizeRussianPhone(
        parsed.data.phone,
      );
      ...
      try {
        ...
        console.log(
          `Заявка №${lead.id} сохранена, письмо отправлено`,
        );
      } catch (mailError) {

        console.error(
          `Заявка №${lead.id} сохранена, ` +
...
app.use((req, res) => {
  return res
    .status(404)
    .sendFile(path.join(PUBLIC_DIR, 'index.html'));
});
...
async function shutdown(signal) {
  console.log(
    `${signal}: завершение работы NADIA server`,
  );
...
app.listen(PORT, async () => {
  console.log(
    `NADIA server started: http://localhost:${PORT}`,
  );
